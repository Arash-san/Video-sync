...
class VlcPlayerWithControlsState extends State<VlcPlayerWithControls>
  ...
  void initState() {
    ...
    widget.eventControl.stream.asBroadcastStream().listen((event) async {
      print("ahhhhhhhhhhh ${event.data}");
      List<String> request = (event.data as String).split("|");
      if (request[0] == "play" && request[1] == "receive") {
        await _controller.play();
      } else if (request[0] == "pause" && request[1] == "receive") {
        await _controller.pause();
      } else if (request[0] == "replay" && request[1] == "receive") {
        await _controller.stop();
        await _controller.play();
      } else if (request[0] == "seekMilisecond") {
        List<String> otherParams = request[1].split("/");
        if (otherParams[1] == "receive") {
          try {
            _controller.setTime(int.parse(otherParams[0]));
          } catch (e) {}
        }
      }
      // else if (request[0] == "seek") {
      //   List<String> otherParams = request[1].split("/");
      //   if (otherParams[1] == "receive") {
      //     try {
      //       _controller.setTime(int.parse(otherParams[0]));
          ...
          child: Container(
            ...
            child: Stack(
              ...
              children: <Widget>[
                ...
                ControlsOverlay(
                  controller: _controller,
                  eventControl: widget.eventControl,
                ),
              ],
            ),
          ),
  ...
  void _togglePlaying() async {
    if (_controller.value.isPlaying) {
      await _controller.pause();
      print("*pause");
      widget.eventControl.sink.add(MyCustomEventType("pause|send"));
    } else {
      await _controller.play();
      print("*play");
      widget.eventControl.sink.add(MyCustomEventType("play|send"));
    }
  }
  ...
  void _onSliderPositionChanged(double progress) {
    ...
    print("*seek to ${sliderValue.toInt() * 1000}");
    widget.eventControl.sink
        .add(MyCustomEventType("seekMilisecond|${sliderValue.toInt() * 1000}/send"));
    _controller.setTime(sliderValue.toInt() * 1000);
  }

  void _getSubtitleTracks() async {
    // if (!_controller.value.isPlaying) return;
    // widget.eventControl.sink.add(MyCustomEventType("babyyyyyyyy"));
